...
func fechar_portas(vizinhos:Array):
	for c in vizinhos:
		...
		if c == "S":
			tilemap.set_cell(Vector2i(7,7),2,Vector2i(0,0),0)
		if c == "L":
			tilemap.set_cell(Vector2i(13,4),2,Vector2i(0,0),0)
		if c == "O":
			tilemap.set_cell(Vector2i(1,4),2,Vector2i(0,0),0)

func abrir_portas(vizinhos:Array):
	vizinhos_instanciados = vizinhos
	for c in vizinhos:
		if c == "N":
			tilemap.set_cell(Vector2i(7,1),1,Vector2i(0,0),0)
		if c == "S":
			tilemap.set_cell(Vector2i(7,7),1,Vector2i(0,0),0)
		if c == "L":
			tilemap.set_cell(Vector2i(13,4),1,Vector2i(0,0),0)
		if c == "O":
			tilemap.set_cell(Vector2i(1,4),1,Vector2i(0,0),0)
